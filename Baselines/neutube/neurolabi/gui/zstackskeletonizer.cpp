...
//  }
  ...
  std::cout << "Finished object loop." << std::endl;

  std::cout << "Killing stackData..." << std::endl;
  C_Stack::kill(stackData);
  stackData = NULL;

  ZSwcTree *wholeTree = NULL;

  std::cout << "Checking regular root..." << std::endl;
  if (Swc_Tree_Regular_Root(tree) != NULL) {
    std::cout << "Creating wholeTree..." << std::endl;
    ...
    if (m_connectingBranch) {
      std::cout << "Connecting branches..." << std::endl;
      reconnect(wholeTree);
    }

    std::cout << "Optimal downsample..." << std::endl;
    ZSwcResampler resampler;
    resampler.optimalDownsample(wholeTree);

    std::cout << "Resort ID..." << std::endl;
    wholeTree->resortId();
